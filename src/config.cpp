...
void validate(const LedaConfig& config) {
  ...
  if (config.dtype != spar::DType::Float32 && config.dtype != spar::DType::Float64) {
  ...
  const auto maximum{static_cast<size_t>(numeric_limits<spar::Shape::dimension_type>::max())};
...
spar::nn::DecoderConfig decoder_config(const LedaConfig& config) {
  validate(config);
...
LedaConfig leda_tiny(size_t vocab_size, spar::DType dtype) {
...
LedaConfig leda_small(size_t vocab_size, spar::DType dtype) {
  LedaConfig config{.vocab_size = vocab_size,
                    .model_dim = 256,
                    .hidden_dim = 768,
                    .num_layers = 8,
