...
bool same_config(const spar::nn::DecoderConfig& left,
                 const spar::nn::DecoderConfig& right) noexcept {
...
Leda::Leda(LedaConfig config, spar::Random& random)
    : config_{std::move(config)}, decoder_{decoder_config(config_), random} {}

Leda::Leda(LedaConfig config, spar::nn::DecoderLM decoder, int)
    : config_{std::move(config)}, decoder_{std::move(decoder)} {}

Leda Leda::from_decoder(LedaConfig config, spar::nn::DecoderLM decoder) {
  const spar::nn::DecoderConfig mapped{decoder_config(config)};
...
spar::Tensor Leda::forward(const spar::Tensor& token_ids, size_t start_position) const {
...
spar::nn::DecoderLM& Leda::decoder() noexcept {
  return decoder_;
}

const spar::nn::DecoderLM& Leda::decoder() const noexcept {
  return decoder_;
}

vector<spar::nn::NamedParameter> named_parameters(Leda& model) {
  return spar::nn::named_parameters(model.decoder());
}

vector<spar::nn::Parameter> parameters(Leda& model) {
  return spar::nn::parameters(model.decoder());
}

void zero_grad(Leda& model) {
  spar::nn::zero_grad(model.decoder());
}
...
ModelStatistics model_statistics(Leda& model) {
  ...
  vector<spar::nn::Parameter> unique;
  for (spar::nn::Parameter parameter : parameters(model)) {
    const bool duplicate{ranges::any_of(unique, [&parameter](const spar::nn::Parameter& existing) {
