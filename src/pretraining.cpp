...
uint64_t target_count(const spar::Tensor& batch) {
...
double scalar_value(const spar::Tensor& tensor) {
  if (tensor.numel() != 1 ||
...
void validate_pretraining_config(const PretrainingConfig& config) {
  ...
  static_cast<void>(spar::training::warmup_cosine_learning_rate(config.learning_rate, 0));
  static_cast<void>(spar::optim::AdamW{{},
...
optional<TrainingStepResult> train_update(Leda& model, spar::optim::AdamW& optimizer,
                                          spar::data::LMBatchIterator& batches,
  ...
  for (; microbatches < config.accumulation_steps; ++microbatches) {
    optional<spar::Tensor> batch{batches.next_batch()};
    if (!batch) {
      break;
    }
  ...
  spar::training::scale_gradients(model_parameters, 1.0 / static_cast<double>(targets));
  const spar::training::ClipGradNormResult clip{
      spar::training::clip_grad_norm(model_parameters, config.max_grad_norm)};
